...
export async function listBoards(req,res,next){try{
  const db=await readDB(); res.json(db.boards);
}catch(e){next(e)}}

export async function getBoard(req,res,next){try{
...
export async function createBoard(req,res,next){try{
...
export async function updateBoard(req,res,next){try{
  const {id}=req.params; const db=await readDB();
...
export async function deleteBoard(req,res,next){try{
  const {id}=req.params; const db=await readDB();
  const idx=db.boards.findIndex(b=>b.id===id);
  if(idx===-1) return res.status(404).json({message:"Board not found"});
